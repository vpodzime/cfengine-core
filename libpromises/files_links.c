...
{
    assert(attr != NULL);
    assert(attr->link.link_type == FILE_LINK_TYPE_SYMLINK);
    ...
    else
    {
        ...
        if (attr->move_obstructions)
        {
            ...
            {
                if (unlink(ChrootChanges() ? ToChangesChroot(destination) : destination) == -1)
                {
...
PromiseResult VerifyAbsoluteLink(EvalContext *ctx, char *destination, const char *source, const Attributes *attr, const Promise *pp)
{
    ...
    {
        bool expanded;
        if (ChrootChanges() && (lstat(ToChangesChroot(absto)) != -1))
        {
